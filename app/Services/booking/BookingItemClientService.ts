...
class BookingItemClientService implements BookingItemClientInterface {
  ...
  async findAll(contract: HttpContextContract) {
    ...
    if (sort_distance == 1 || (lng && lat)) {
      ...
      const filtered = this.sortItemListByDistance(
        arr,
        sort_distance == 1,
        lat,
        lng,
  ...
  async sortItemListByDistance(
    ...
    if (km) {
      ...
      filteredArr = filteredArr.filter((p) => {
        if (p.isVirtual && p.geolocation === null) return false;
        ...
        console.log(
          `Distance(km): ${getDistance(origin, geoObj) / 1000}, radius: ${
            km * 1000
          }, inPoint: ${isIn}`
        );
        return isIn;
    ...
    if (sortDistance) {
      const sortedArr = orderByDistance(
        origin,
        items
          .filter((p) => p.isVirtual !== 1 || p.geolocation)
          .map((p) => {
            const geoObj = JSON.parse(p.geolocation);
  ...
  async findById({ request, auth }: HttpContextContract) {
    ...
    if (user !== null && isLoggedIn) {
      ...
      await user.goocard.load("coupons", (q) => {
